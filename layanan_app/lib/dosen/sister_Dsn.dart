...
import 'package:layanan_app/model/dosen_model/sisterDsn_model.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:http/http.dart' as http;

var selectedService = 0;

class sisterDsn extends StatefulWidget {
  final status;
  sisterDsn({this.status});

  @override
  State<sisterDsn> createState() => _sisterDsnState();
}

class _sisterDsnState extends State<sisterDsn> {
  @override
  void initState() {
    super.initState();
    _loadReceivedData();
  ...
  Future<void> _loadReceivedData() async {
    ...
    if (_receivedData != "No data" && !JwtDecoder.isExpired(_receivedData)) {
      await addDataUserSister(); // Wait for this to complete
      await getDataSister(); // Only call this after the previous function completes
    }
  }

  Future<void> addDataUserSister() async {
    var response = await http.get(
      Uri.parse(AksesSister.UrlSister_Sp + 'api/VerifyToken/verify_token/'),
  ...
  Future<void> getDataSister() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
      final response = await http.get(Uri.parse(
          AksesSister.UrlSister_Sp + "api/DataSister/index?email=$email"));
  ...
  Container _menuItems(sisterDsnModel sisterDsnModel) {
    return Container(
        ...
        child: Row(children: [
          Container(
            ...
            child: Image.asset(
              'assets/sisterFitur_Image/${sisterDsnModel.image}',
              width: 110,
              height: 110,
            ),
          ),
          ...
          Flexible(
            ...
            child: Column(
              ...
              children: [
                ...
                RichText(
                    text: TextSpan(
                        text: "Layanan: ${sisterDsnModel.ket}",
                        style: GoogleFonts.manrope(
    ...
    return AspectRatio(
      ...
      child: Container(
        ...
        child: Stack(
          children: [
            Image.asset(
              'assets/layananImage/dsn_sister.png',
