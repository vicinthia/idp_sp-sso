...
import 'package:layanan_app/model/dosen_model/kknDsn_model.dart';
...
class kknDsn extends StatefulWidget {
  @override
  State<kknDsn> createState() => _kknDsnState();
}

class _kknDsnState extends State<kknDsn> {
  @override
  void initState() {
    super.initState();
    _loadReceivedData();
  ...
  Future<void> _loadReceivedData() async {
    ...
    if (_receivedData != "No data" && !JwtDecoder.isExpired(_receivedData)) {
      await addDataUserKKN(); // Wait for this to complete
      await getDataKKN(); // Only call this after the previous function completes
    }
  }

  Future<void> addDataUserKKN() async {
    var response = await http.get(
      Uri.parse(AksesKKN.UrlKKN_Sp + 'api/VerifyToken/verify_token/'),
  ...
  Future<void> getDataKKN() async {
    ...
    try {
      final response = await http.get(
          Uri.parse(AksesKKN.UrlKKN_Sp + "api/DataKKN/index?email=$email"));
  ...
  ListView _menuItem() {
    return ListView.separated(
        ...
        shrinkWrap: true,
        itemBuilder: (context, index) => _menuItems(menuKKN[index]),
        separatorBuilder: (context, index) => SizedBox(
              height: 11,
            ),
        itemCount: menuKKN.length);
  }

  Container _menuItems(kknDsnModel kknDsnModel) {
    return Container(
        ...
        child: Row(children: [
          Container(
            ...
            child: Image.asset(
              'assets/kknFitur_Image/${kknDsnModel.image}',
              width: 110,
              height: 110,
            ),
          ),
    ...
    return AspectRatio(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: Color.fromARGB(255, 155, 37, 165),
        ),
        child: Stack(
          children: [
            Image.asset(
              'assets/layananImage/kkn_board.png',
                ...
                children: [
                  RichText(
                      text: TextSpan(
                          ...
                          children: [
                        TextSpan(
                            text: "Sistem Kuliah Kerja Nyata",
                            style: TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.w800)),
                        TextSpan(
                            text:
                                "\nPenyedia layanan kegiatan pengabdian mahasiswa\nkampus online")
                      ])),
                  SizedBox(
