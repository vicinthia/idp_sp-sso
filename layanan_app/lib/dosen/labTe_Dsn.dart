...
import 'package:layanan_app/model/dosen_model/labTeDsn_Model.dart';
...
class labTEDsn extends StatefulWidget {
  @override
  State<labTEDsn> createState() => _labTEDsnState();
}

class _labTEDsnState extends State<labTEDsn> {
  @override
  void initState() {
    super.initState();
    _loadReceivedData();
  ...
  Future<void> _loadReceivedData() async {
    ...
    if (_receivedData != "No data" && !JwtDecoder.isExpired(_receivedData)) {
      await addDataUserLabTe(); // Wait for this to complete
      await getDataLabTe(); // Only call this after the previous function completes
    }
  }

  Future<void> addDataUserLabTe() async {
    var response = await http.get(
      Uri.parse(AksesLabTe.UrlLabTe_Sp + 'api/VerifyToken/verify_token/'),
  ...
  Future<void> getDataLabTe() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
      final response = await http.get(Uri.parse(
          AksesLabTe.UrlLabTe_Sp + "api/DataLabTe/index?email=$email"));
  ...
  ListView _menuItem() {
    return ListView.separated(
        ...
        shrinkWrap: true,
        itemBuilder: (context, index) => _menuItems(menuItem[index]),
  ...
  Container _menuItems(labTeDsnModel labTeDsnModel) {
    return Container(
        ...
        child: Row(children: [
          Container(
            ...
            child: Image.asset(
              'assets/labFitur_Image/${labTeDsnModel.image}',
              width: 110,
              height: 110,
            ),
          ),
          ...
          Flexible(
            ...
            child: Column(
              ...
              children: [
                ...
                RichText(
                    text: TextSpan(
                        text: "Layanan: ${labTeDsnModel.ket}",
                        style: GoogleFonts.manrope(
    ...
    return AspectRatio(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: Color.fromARGB(255, 24, 9, 167),
        ),
        child: Stack(
          children: [
            Image.asset(
              'assets/layananImage/labte.png',
                ...
                children: [
                  RichText(
                      text: TextSpan(
                          ...
                          children: [
                        TextSpan(
                            text: "Laboratorium Teknik Elektronika ",
                            style: TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.w800)),
                        TextSpan(
                            text:
                                "\nPenyedia layanan kegiatan praktikum\nelektronika kampus online")
                      ])),
                  SizedBox(
        ...
        children: [
          Text(
            userDetailsText.isNotEmpty
                ? 'LAB ELEKTRONIKA, $userDetailsText!'
                : 'Loading data...',
            style: GoogleFonts.manrope(
                fontSize: 24,
                fontWeight: FontWeight.w800,
