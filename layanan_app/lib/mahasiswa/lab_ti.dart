...
import 'package:layanan_app/mahasiswa/fiturLab/formRegis.dart';
import 'package:layanan_app/model/labti_model.dart';
...
class labTIMhs extends StatefulWidget {
  @override
  State<labTIMhs> createState() => _labTIMhsState();
}

class _labTIMhsState extends State<labTIMhs> {
  @override
  void initState() {
    super.initState();
    _loadReceivedData();
  ...
  Future<void> _loadReceivedData() async {
    ...
    if (_receivedData != "No data" && !JwtDecoder.isExpired(_receivedData)) {
      await addDataUserLabTi(); // Wait for this to complete
      await getDataLabTi(); // Only call this after the previous function completes
    }
  }

  Future<void> addDataUserLabTi() async {
    var response = await http.get(
      Uri.parse(AksesLabTi.UrlLabTi_Sp + 'api/VerifyToken/verify_token/'),
  ...
  Future<void> getDataLabTi() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
      final response = await http.get(Uri.parse(
          AksesLabTi.UrlLabTi_Sp + "api/DataLabTi/index?email=$email"));
  ...
  ListView _menuItem() {
    return ListView.separated(
        ...
        shrinkWrap: true,
        itemBuilder: (context, index) => _menuItems(menuLabTi[index], context),
        separatorBuilder: (context, index) => SizedBox(
              height: 11,
            ),
        itemCount: menuLabTi.length);
  }

  // Container _menuItems(labTiModel labTiModel) {
  //   return
  GestureDetector _menuItems(labTiModel labTiModel, BuildContext context) {
    return GestureDetector(
        onTap: () {
          switch (labTiModel.nama) {
            case "Daftar Praktikum":
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => formPraktikum(
                          userNim: userNim,
        ...
        child: Container(
            ...
            child: Row(children: [
              Container(
                ...
                child: Image.asset(
                  'assets/labFitur_Image/${labTiModel.image}',
                  width: 110,
                  height: 110,
                ),
              ),
    ...
    return AspectRatio(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: Color.fromARGB(255, 23, 197, 102),
        ),
        child: Stack(
          children: [
            Image.asset(
              'assets/layananImage/labti.png',
                ...
                children: [
                  RichText(
                      text: TextSpan(
                          ...
                          children: [
                        TextSpan(
                            text: "Laboratorium Informatika ",
                            style: TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.w800)),
                        TextSpan(
                            text:
                                "\nPenyedia layanan kegiatan praktikum\ninformatika kampus online")
                      ])),
                  SizedBox(
